...
```
Source → inbox/ (raw) → /arscontexta:extract → notes/ (claims) → /arscontexta:connect → linked claims → /arscontexta:verify → validated claims
```
...
**Failure mode to watch:** Collector's Fallacy. It is easy to keep accumulating sources without extracting. Extract before you capture more. Check inbox depth with `/arscontexta:stats`.

### Stage 2: Extract

Run `/arscontexta:extract` on a raw source. The system reads the full source and identifies atomic claims -- individual propositions that can stand alone.
...
Run `/arscontexta:connect` on extracted claims. This is where cross-disciplinary value emerges.
...
Run `/arscontexta:verify` on connected claims to check accuracy and evidence quality. Verification may:
...
For end-to-end processing, use `/arscontexta:pipeline`:

```
/arscontexta:pipeline inbox/rust-audio-buffer-lifetimes.md
```

This chains extract, connect, and verify in sequence. For more control, run each stage manually.

For automated sequencing, use `/arscontexta:ralph` -- it analyzes vault state and runs whatever stage needs attention most.
...
Claims extracted but never connected to topic maps. They exist in notes/ but are invisible to navigation. `/arscontexta:stats` reports orphan count. `/arscontexta:reweave` integrates them.

**Prevention:** `/arscontexta:extract` should assign topic map links during extraction. If orphans accumulate, the extraction prompts may need tuning via `/arscontexta:learn`.

**Cure:** Run `/arscontexta:reweave` when orphan count exceeds ~10% of total claims.
...
**Prevention:** Periodic `/arscontexta:connect` passes over older claims.

**Cure:** `/arscontexta:connect --stale` to re-examine claims that have not been connected since creation.
...
**Prevention:** `/arscontexta:verify` checks claims against current source state.

**Cure:** `/arscontexta:rethink` on stale claims. May result in updated content, revised evidence, or archival with replacement. See [[meta-skills]] for /arscontexta:rethink patterns.

### Maintenance Triggers

Run maintenance when `/arscontexta:stats` shows any of:
...
Or simply run `/arscontexta:ralph` and let it decide.
...
1. `/arscontexta:stats` -- Check vault health
2. `/arscontexta:next` -- Get a recommendation
3. Do one thing: extract one source, or connect a batch of recent claims, or verify a few stale claims
4. `/arscontexta:validate` -- Quick structural check before ending

### Deep Session (1-2 hours)

Best for substantial extraction work or structural maintenance.

1. `/arscontexta:stats` -- Assess vault state
2. `/arscontexta:tasks` -- Review the full work queue
3. **Phase 1: Extract.** Process 2-3 inbox sources with `/arscontexta:extract`. This produces 10-30 new claims.
4. **Phase 2: Connect.** Run `/arscontexta:connect` on each new claim. Look for cross-disciplinary bridges.
5. **Phase 3: Maintain.** If orphans accumulated, run `/arscontexta:reweave`. If stale claims surfaced, run `/arscontexta:rethink` on the most important ones.
6. `/arscontexta:validate` -- Full structural check
7. `/arscontexta:stats` -- Compare to session start. Note what improved.

### Review Session (30-60 minutes)

Best for stepping back and assessing the vault's trajectory.

1. `/arscontexta:graph --global` -- Look at the overall topic map structure
...
5. `/arscontexta:rethink` on any claims that feel outdated or underspecified
