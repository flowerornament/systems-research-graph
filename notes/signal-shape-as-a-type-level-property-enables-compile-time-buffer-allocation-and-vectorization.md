---
description: When matrix shape (rows x columns) is part of the signal's type rather than runtime metadata, the compiler can allocate buffers, plan SIMD vectorization, and detect shape mismatches before any audio runs
type: claim
evidence: moderate
