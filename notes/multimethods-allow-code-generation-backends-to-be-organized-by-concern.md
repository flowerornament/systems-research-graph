...
Wadler's formal statement in [[the-expression-problem-names-the-tension-between-adding-new-cases-and-new-operations-without-recompiling]] provides the canonical constraint set: multimethod dispatch solves the operations (columns) dimension cleanly but still requires one method per backend per new case -- the cases dimension remains an O(backends × cases) obligation. [[the-visitor-pattern-solves-the-operations-dimension-of-the-expression-problem-but-not-the-cases-dimension]] identifies the same asymmetry in the visitor pattern; multimethods improve organization within the operations dimension without fundamentally resolving the cases dimension tension.
