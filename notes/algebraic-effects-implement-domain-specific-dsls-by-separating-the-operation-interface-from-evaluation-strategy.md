...
The structural analogy to [[caching-ugen-graph-evaluation-order-decouples-topology-modification-from-concurrent-execution]] is close: the cached evaluation order IS a kind of handler -- it defines *how* the synthesis graph's abstract "evaluate this node" operations are executed (sequentially, vectorized, in what order). The graph itself is effect-polymorphic in this sense: the same topological structure can be evaluated by a real-time cached-order handler, a test handler that captures outputs to a buffer, or an analysis handler that computes dependency depth. Algebraic effects formalize what the synthesis graph pattern implements informally.
