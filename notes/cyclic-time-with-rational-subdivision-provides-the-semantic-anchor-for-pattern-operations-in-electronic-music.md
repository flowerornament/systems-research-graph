...
The cyclic model also contrasts with [[suspending-time-until-explicitly-advanced-gives-deterministic-reproducible-timing-across-machines]] (ChucK): ChucK's suspended-animation timing creates deterministic time control by letting the programmer explicitly advance `now`, but without a named cycle concept -- the "cycle" in ChucK is whatever period the programmer chooses, making reversal and phase-shift operations user-defined rather than language-primitives. Tidal makes cycles the *universal semantic unit*: every pattern operation is defined relative to the cycle, giving the language a fixed reference frame that ChucK's approach deliberately omits in favor of programmer control. This is a fundamental design divergence -- Tidal's cyclic anchor enables richer compositional semantics at the cost of constraining how time is structured; ChucK's unconstrained `now` enables arbitrary temporal programming at the cost of no built-in periodic semantics.
