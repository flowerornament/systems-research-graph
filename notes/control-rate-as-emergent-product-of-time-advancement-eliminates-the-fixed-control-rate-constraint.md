...
This is the language-semantic inversion of [[synthesis-function-specialization-by-input-rate-multiplies-performance-without-multiplying-algorithm-complexity]]: where SC1 specializes synthesis functions into variants for statically-classified input rates (the compiler chooses which variant to dispatch), ChucK makes rate itself a programmer-controlled variable so no pre-classification is needed -- but also no compile-time optimization is possible. The two designs sit at opposite ends of the static/dynamic rate tradeoff: SC1's 12 Aoscilia variants extract maximum performance at the cost of implementation burden; ChucK's emergent rate buys flexibility at the cost of always running the general case.
