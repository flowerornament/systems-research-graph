...
A real-time audio effect plugin with 10 ms latency tolerance at 48 kHz has ~480 samples of budget. A plugin requiring sub-millisecond latency has ~48. The former can use substantially larger model inputs with proportionally lower per-sample cost. This means latency budget is itself a model architecture constraint — teams designing neural audio effects should consider the target latency requirement before fixing the model's input shape. The tradeoff is structurally parallel to [[block-size-is-the-primary-latency-throughput-tradeoff-in-murail-execution]]: at the substrate level, larger block size N improves throughput via vectorization at the cost of latency; at the inference scheduling level, larger model input size improves per-sample efficiency at the cost of H_adapt. Both are instances of the same design pattern — batching more work per invocation to amortize fixed overhead — operating at different layers of the audio system.
