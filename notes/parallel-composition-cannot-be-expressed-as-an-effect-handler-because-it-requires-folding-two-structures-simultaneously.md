...
ChucK's concurrency model ([[user-space-cooperative-shreds-achieve-sample-accurate-deterministic-concurrency-without-os-scheduling]]) sidesteps this problem by avoiding the need to express parallelism as an effect: shreds yield cooperatively at explicit time-advance points, so at any given moment only one shred is executing. The "parallel" behavior emerges from interleaving, not from genuine simultaneous execution, which is why it can be encoded in a sequential system without binary handlers. This is the practical price of ChucK's tractability -- true CCS-style parallel composition with rendezvous synchronization cannot be expressed in ChucK any more than it can in algebraic effects.
