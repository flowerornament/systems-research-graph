...
**FAUST nuance:** FAUST resolves the runtime introspection problem differently — not by preserving a running image, but by making the program's meaning explicit in its formal semantics. [[faust-programs-denote-mathematical-functions-enabling-semantics-driven-compilation]] shows that because a FAUST program denotes a mathematical function, its meaning can be queried at compile time (via documentation generation) even though the compiled artifact is fully static. This is a compensation strategy, not a solution: you can understand what the program *computes*, but you cannot inspect what it is *doing at runtime*.
