...
Steele's criterion ([[user-defined-extensions-must-be-syntactically-indistinguishable-from-built-in-primitives]]) adds the growth dimension: a high-level language that formalizes many constraints is only beneficial if those constraints can be *extended by users* without syntactic seams. A maximally expressive language with a closed constraint vocabulary merely shifts the informal program language from "how do I use this type correctly" to "why can't I express this concept in the type system." The goal is a language whose formal expressiveness grows with user needs, not one that trades one kind of informal overhead for another.
