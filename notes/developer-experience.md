...
- [[live-coding-performance-requires-three-distinct-feedback-loops-that-together-define-being-in-time]] -- McLean 2014: manipulation (code<>programmer), performance (sound<>programmer), social (audience<>programmer) feedback loops must all be active for live coding to constitute "being in time"; each loop imposes distinct latency requirements
- [[low-cognitive-load-and-high-viscosity-in-a-composition-language-enable-creative-flow-in-live-performance]] -- Green's cognitive dimensions applied to live music coding: Tidal's terseness and composability enable flow states during performance by minimizing working memory requirements and maximizing expressiveness per keystroke
- [[a-haskell-embedded-dsl-can-be-learned-and-used-creatively-without-any-functional-programming-background]] -- 12/15 Tidal users had no FP experience; 14/15 could make music; the EDSL abstraction barrier is what enables accessibility, not simplifying the host language
- [[iterative-redesign-of-core-representations-through-live-performance-is-the-correct-evaluation-methodology-for-music-programming-languages]] -- Tidal's 5+ representation rewrites each evaluated through live performance; performance pressure is the correct evaluation context for music language design decisions
