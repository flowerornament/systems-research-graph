...
### Tidal Cycles (McLean 2014): Functional Live Coding Language
- [[functional-paradigm-dominates-live-coding-practice-because-it-provides-terseness-and-composability-at-performance-speed]] -- empirical observation across live coding systems (Overtone, Fluxus, Extempore, Tidal, Conductive): functional/Lisp-based environments dominate performance contexts because they have highest viscosity (expressiveness per keystroke) under pressure
- [[representing-musical-pattern-as-a-function-from-time-to-events-unifies-discrete-and-continuous-patterns]] -- Tidal's `Pattern a = Arc -> [Event a]`; single type handles discrete sequences and continuous signals, all combinators compose freely; eliminates the Behavior/Event split that complicates FRP-based designs
- [[rational-time-representation-eliminates-rounding-errors-in-rhythmic-subdivision]] -- `type Time = Rational`; triplets, compound meters, and polyrhythms computed exactly; floating-point time corrupts long-running rhythmic patterns
- [[cyclic-time-with-rational-subdivision-provides-the-semantic-anchor-for-pattern-operations-in-electronic-music]] -- the cycle (indexed by whole number) is the semantic unit; reversal, phase-shift, conditional transformations all defined relative to cycle; aligns with Indian classical sam concept and EDM downbeat
- [[applicative-functor-lifts-binary-value-functions-to-pattern-combinators-by-matching-co-occurring-events]] -- `Pattern` as Applicative; any binary value function becomes a pattern combinator via `f <$> p1 <*> p2`; matching by onset containment preserves left-pattern temporal structure
- [[a-haskell-embedded-dsl-can-be-learned-and-used-creatively-without-any-functional-programming-background]] -- 14/15 Tidal users could make music despite most having no FP experience; the EDSL abstraction barrier is the accessibility lever
