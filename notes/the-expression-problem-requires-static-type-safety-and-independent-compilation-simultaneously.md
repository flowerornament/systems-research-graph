...
The connection to [[effects-as-capabilities-can-encode-rt-safety-requirements-in-the-composition-language-type-system]] is that *type system expressiveness* is the shared requirement: both the expression problem and RT capability encoding require a type system that can represent relationships (between cases and operations; between effects and their handlers) without runtime casts. An expressive enough type system dissolves both problems simultaneously -- adding a new UGen type (new row) and a new analysis pass (new column) are just new capability declarations if the type system can express the capability algebra.
