...
For murail's neural UGen design, this constraint narrows the decision space: models intended for effects with persistent tonal or dynamic character (amp sims, reverb tails, compressors) must use LibTorch or TensorFlow Lite. ONNX Runtime is appropriate only for models where each audio buffer can be processed independently.
