...
Connects forward to [[evolvability-requires-trading-provability-for-extensibility]]: if specs can't fully specify meaning, tight specs are doubly brittle. Connects to [[hirams-law-makes-all-observable-interpreter-behavior-a-permanent-api-commitment]]: users will rely on the full observable behavior of a system because they have no way to know which behaviors were intentional from the source text alone.

Baniassad & Myers ([[baniassad-myers-2009-program-as-language]]) give a structural account of what "meaning not in the source" consists of: [[program-languages-bridge-the-gap-between-program-semantics-and-code-semantics]]. The "gap" between program semantics (what the code means to humans) and code semantics (what the machine executes) is precisely what source text cannot specify. This gives Naur's claim a more precise linguistic framing without contradicting it.
