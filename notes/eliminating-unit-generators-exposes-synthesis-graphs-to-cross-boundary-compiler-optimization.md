---
description: When UGens are library functions over four primitives (constants, math, delays, control flow) rather than opaque objects, the compiler sees the full graph and can fuse, vectorize, and dead-strip
type: claim
evidence: moderate
...
**Prior art:** FAUST's architecture is a direct precedent. [[faust-programs-denote-mathematical-functions-enabling-semantics-driven-compilation]] shows that this approach works in production: FAUST's compiler compiles the mathematical function the block-diagram denotes, not the block-diagram literally. [[purely-functional-dsp-semantics-enables-compiler-optimizations-impossible-in-C]] explains precisely why — the absence of aliasing and side effects removes the barriers that block C++ compilers from cross-boundary optimization.

The UGen-as-opaque-object model that this claim replaces traces directly to SC1's closure-based UGens: [[closures-encapsulate-unit-generator-state-making-ugen-creation-a-function-call-not-a-class-definition]]. Closures encapsulate state ergonomically but are opaque to the optimizer; McCartney's new system replaces closure opacity with explicit primitive node IR, enabling cross-boundary visibility.
