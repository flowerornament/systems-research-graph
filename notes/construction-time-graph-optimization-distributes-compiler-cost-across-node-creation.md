---
description: Applying constant folding, CSE, rate inference, and 120+ rewrite rules at node creation rather than post-construction distributes optimization cost and delivers a pre-simplified graph to the compiler
type: claim
evidence: moderate
...
Warning from Hadron: [[constant-folding-can-silently-change-sc-program-semantics-via-initialization-timing]] shows that in SC, constant folding is not semantics-preserving because deferred initialization timing is observable. Murail must explicitly classify which optimization transforms are safe under its compatibility contract before applying them at construction time.

The rate inference step (constant < init < event < audio) formalizes the informal dispatch that [[synthesis-function-specialization-by-input-rate-multiplies-performance-without-multiplying-algorithm-complexity]] describes via SC1's 12-variant Aoscilia oscillator: instead of hand-written variants, construction-time rate inference identifies the appropriate optimization automatically.
