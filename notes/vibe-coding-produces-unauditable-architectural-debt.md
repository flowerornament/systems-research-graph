...
Naur's [[programming-is-theory-building-not-text-production]] provides the theoretical grounding: vibe coding produces text without the programmer ever having built the theory, which puts the codebase immediately into the "revival by newcomers" condition. [[program-revival-by-newcomers-systematically-produces-degraded-designs]] describes what happens next: each subsequent modification degrades the design because no one holds the theory. Lattner independently rediscovered Naur's 1985 insight.
