...
The live coding context makes this especially acute: a musician porting a Tidal pattern to ChucK is not translating code but translating between two distinct program languages. Tidal's cyclic time model ([[cyclic-time-with-rational-subdivision-provides-the-semantic-anchor-for-pattern-operations-in-electronic-music]]) is a program-language construct -- the meaning of "pattern," "cycle," and "subdivision" in Tidal are specific to Tidal's program language and do not translate directly to ChucK's shred-and-now model ([[suspending-time-until-explicitly-advanced-gives-deterministic-reproducible-timing-across-machines]]). Both express "music over time" but in mutually untranslatable program languages built on incompatible temporal semantics.
