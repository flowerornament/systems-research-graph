...
McCartney's recommendation to read multiple audio engines ([[audio-programming-education-requires-reading-production-engine-source-code-across-multiple-systems]]) is implicit recognition of this: reading CSound, ChucK, SuperCollider, and PD means acquiring five distinct program languages, each encoding different design decisions about RT safety, graph topology, and timing. The multi-system approach is effective precisely because each system's program language reveals the design choices that a single system's API obscures -- each system must "say the same thing" (audio processing) in a different program language, making the variation visible.
