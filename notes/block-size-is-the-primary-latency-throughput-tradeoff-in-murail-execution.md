...
- The same latency-throughput pattern recurs at the neural inference level: [[larger-model-input-sizes-improve-per-sample-inference-performance-making-latency-tolerance-a-throughput-lever]] shows that larger model input sizes amortize per-sample inference overhead, paralleling how larger block sizes amortize per-tick vectorization overhead
- [[pre-computed-event-buffers-with-constant-time-offset-trade-added-latency-for-reduced-jitter]] names this as a general RT pattern: block rendering is event buffering at sample granularity, and N samples of latency is the "constant time offset" that absorbs scheduling jitter
- [[signal-buffers-as-a-type-amortize-interpreter-overhead-making-high-level-languages-viable-for-real-time-dsp]] is the original insight this block-size tradeoff flows from: the buffer (control period) is the unit of interpreter dispatch in SC1; murail's block size is the same concept formalized as a substrate tuning parameter
