...
SuperCollider's design lineage is the audio domain instance: McCartney built SC1 on a Smalltalk-inspired, Lisp-adjacent language precisely because synthesis expressions and compositional algorithms should share identical syntax ([[scripting-and-synthesis-in-the-same-language-eliminates-the-boundary-between-composition-and-sound-design]]). The "composing is programming" claim is the audio equivalent of Steele's "all primitives look like user definitions" -- when synthesis functions and composition scripts share a language that makes no syntactic distinction between them, the creative space is fully explorable without boundary. Overtone (Clojure), Tidal (Haskell), and SuperCollider all inherit this insight from Lisp's user-driven growth model.
