---
description: Guarded domain theory and Murail's causality invariant share one principle: every cycle must pass through a "later" guard -- connecting formal foundations to compiler enforcement
type: claim
evidence: moderate
...
- GCTT ([[birkedal-2016-guarded-cubical-type-theory]]) extends this picture: if Murail programs were typed in a GCTT-like system, not only would the causality checker enforce guardedness (as now), but the path equality mechanism would enable *proving that two programs compute equivalent output streams* — formal program equivalence for audio graphs; [[path-types-in-gctt-enable-proving-properties-of-guarded-recursive-programs-not-just-defining-them]] demonstrates this for stream programs
