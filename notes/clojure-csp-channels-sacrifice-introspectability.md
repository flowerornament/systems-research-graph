...
Contrast with [[algebraic-effects-model-async-without-async-await-keywords-by-registering-the-continuation-as-a-callback]]: algebraic effect handlers implement the same async-suspend-resume pattern but keep the continuation explicit as the `resume` variable in the handler clause. The handler stack is inspectable at runtime; the suspended continuation has a name. This is the structural advantage Leijen's approach has over CSP channels -- the abstraction does not compile away its structure.
