...
This cache coherency problem is a specific instance of the broader constraint described in [[memory-bandwidth-is-the-binding-constraint-for-audio-dsp-parallelism-on-smp-machines]]: lock metadata writes consume bus bandwidth that would otherwise be available for DSP data. The problem is compounded by [[cache-hierarchical-hardware-makes-linked-lists-slower-than-arrays-for-most-practical-sizes]] -- if lock-protected nodes are scattered across cache lines, each lock acquisition pollutes the cache with non-DSP data. [[pervasive-immutability-dissolves-concurrency-problems-rather-than-managing-them]] eliminates the problem at its root: immutable data needs no locks, so read-only access generates zero write traffic by construction.
