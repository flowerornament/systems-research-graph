---
description: In McCartney's system, a synthesis definition is built by calling ordinary functions that create nodes; there is no distinct synthdef syntax -- the full language is available for graph construction
type: claim
evidence: moderate
...
This claim is the culmination of a trajectory that begins with [[closures-encapsulate-unit-generator-state-making-ugen-creation-a-function-call-not-a-class-definition]] (SC1, 1996): even then, UGen creation was already a function call in the SC language. The shift from SC1 closures to tau5 primitive nodes preserves the "UGen = function call" user experience while replacing the runtime dispatch mechanism with static graph IR.
