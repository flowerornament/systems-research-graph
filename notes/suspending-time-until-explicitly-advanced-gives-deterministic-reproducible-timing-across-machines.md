...
The suspended-animation model is also the foundation for ChucK's reproducibility claim: since timing does not depend on hardware or OS, a ChucK program is, in principle, a reproducible artifact. This is the property that [[dataflow-languages-lack-explicit-formal-semantics-making-program-behavior-engine-dependent]] identifies as missing from Max/PD -- ChucK's `now` semantics provides a formal, language-level grounding for when things happen. Tidal takes a complementary approach to the same reproducibility goal: where ChucK's `now` eliminates OS-scheduler non-determinism at the *control* level, [[rational-time-representation-eliminates-rounding-errors-in-rhythmic-subdivision]] eliminates floating-point accumulation at the *arithmetic* level. Together, the two techniques address the two independent sources of temporal drift in music systems.
