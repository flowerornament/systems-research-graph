...
SAPF extends this further in two directions. [[sapf-append-only-execution-log-provides-ten-year-session-provenance]] is the most radical implementation of property (2) in the vault: a decade-long append-only record of every executed expression means no creative session is ever lost, extending "no blank-slate startup" from session-scoped to career-scoped. And [[lazy-infinite-lists-enable-sample-level-access-and-sonic-composting-in-signal-graph-languages]] demonstrates property (3) applied to audio: the inspect-before-play workflow lets the composer examine sample values before rendering, making the audio signal itself introspectable during development rather than only audible after execution.
