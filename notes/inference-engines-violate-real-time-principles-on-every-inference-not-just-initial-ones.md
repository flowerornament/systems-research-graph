...
For murail specifically, the violation categories map directly onto substrate design constraints. The malloc/free violations justify [[all-murail-program-state-fits-in-a-single-pre-allocated-contiguous-region]] — the pre-allocated state region Sigma eliminates heap allocation from the fast thread entirely. The pthread_mutex_lock violations are a concrete instance of the cache-coherency penalty documented in [[read-only-lock-operations-cause-cache-coherency-traffic-even-on-unmodified-data]], and the substrate's response is [[hold-slots-prevent-fast-thread-blocking-on-slow-rate-values-without-locks]], which provides lock-free cross-rate reads. The fast-thread never-halt axiom ([[the-murail-fast-thread-never-halts-errors-degrade-quality-but-do-not-stop-evaluation]]) is the formal statement of the constraint that these violations make necessary.
