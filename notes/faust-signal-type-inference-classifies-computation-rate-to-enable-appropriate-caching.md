...
- Historical predecessor: [[synthesis-function-specialization-by-input-rate-multiplies-performance-without-multiplying-algorithm-complexity]] (SC1 1996) solved the same problem through runtime dispatch tables (12 Aoscilia variants); FAUST solved it through static type inference (single source, rate-classified compilation). The progression shows the same insight formalized from runtime to compile-time: McCartney did it manually and by hand; FAUST automated it through type-directed compilation
