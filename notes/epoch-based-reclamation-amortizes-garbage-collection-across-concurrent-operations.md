...
**Implications for murail:** For the NRT side (non-real-time), EBR is appropriate -- the NRT thread can stall without catastrophe, and the low per-operation overhead is valuable. For the RT side, neither EBR nor SMR are suitable since both require periodic scanning. The preferred pattern is to avoid dynamic allocation on the RT thread entirely. This aligns with [[rust-provides-provably-memory-and-data-race-safe-programs-without-garbage-collection]]: Rust's no-GC guarantee means EBR is one of the few viable NRT reclamation strategies that integrates cleanly with Rust's ownership model.

In a system built on [[pervasive-immutability-dissolves-concurrency-problems-rather-than-managing-them]], EBR's role narrows: immutable objects can be freely shared without synchronization, so EBR is needed only for the mutable coordination structures (e.g., the graph topology itself during compile-and-swap). See [[reference-counting-becomes-viable-when-immutability-prevents-object-cycles]] for the complementary approach where immutability eliminates the reference cycle problem that normally disqualifies reference counting.
