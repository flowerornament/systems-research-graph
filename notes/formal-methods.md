...
- Can GCTT's path equality be used to formalize audio program equivalence for Murail? [[path-types-in-gctt-enable-proving-properties-of-guarded-recursive-programs-not-just-defining-them]] shows that stream function commutativity is provable in GCTT. Murail programs compute streams; if they can be expressed in a GCTT-like type theory, path equality would give a formal criterion for when two audio graphs produce equivalent output — relevant for optimization, refactoring, and formal compilation correctness.
- GCTT currently lacks clock quantification ([[gctt-excludes-clock-quantification-from-gdtt-making-coinductive-types-unrepresentable-as-first-class-types]]). What would clock quantification unlock for Murail's formal model? The current Murail substrate already has a notion of "signal time" that is separate from program time; could this map to clock variables?
