---
description: Immutable data with explicit mutable Refs eliminates GC write barriers, makes RC sufficient, enables lock-free sharing, and makes undo a pointer swap -- all as side effects of one decision
type: claim
evidence: strong
...
Lattner frames the same argument from the ownership angle: [[value-semantics-allow-in-place-mutation-when-ownership-is-clear-making-them-strictly-more-powerful-than-purely-functional-copies]] shows that when ownership is statically known, in-place mutation is safe and efficient -- making Rust's model and McCartney's immutable-first model complementary rather than conflicting. The Murail composition language question above may dissolve: Rust ownership gives you in-place mutation where safe; persistent structures give you sharing and undo where ownership is not exclusive.
