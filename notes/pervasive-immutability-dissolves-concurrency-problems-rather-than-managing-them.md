...
**Question for Murail (Stage 9 composition language):** Murail's engine is in Rust, which already has ownership-based memory safety. But the composition language needs its own memory model. Should it follow McCartney's lead (persistent immutable structures with explicit mutable refs) or leverage Rust's ownership system differently? The persistent-structure approach has proven advantages for undo/versioning and concurrency; Rust ownership has proven advantages for zero-overhead performance. These might combine: Rust for the engine, persistent structures for the language runtime's data model.
