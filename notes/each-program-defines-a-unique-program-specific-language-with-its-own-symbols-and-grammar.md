...
The relationship to [[a-language-must-be-designed-to-grow-not-to-be-complete]] is direct but inverted: Steele argues that the formal programming language must provide mechanisms for user-driven growth so that users can define new vocabulary; Baniassad & Myers observe that every program *already* defines such vocabulary -- the program language -- whether the programming language supports it gracefully or not. A language designed to grow (Steele) makes the program language's implicit vocabulary explicit and composable; a language that does not grow forces the program language to carry more informal, fragile meaning. The less the formal language can express, the heavier the program language burden.
