...
For murail, this architecture maps directly onto the substrate's formal real-time guarantees. The substrate's resource invariant (Axiom 12.2) forbids heap allocation, locks, I/O, and unbounded-time calls during TICK — precisely the violations inference engines produce. Since [[all-murail-program-state-fits-in-a-single-pre-allocated-contiguous-region]], any neural UGen that executes inference inline would violate the pre-allocation invariant. The thread pool is the necessary architectural boundary: inference engines live outside the state region, and only their results enter the fast thread via lock-free handoff. Where [[the-murail-fast-thread-never-halts-errors-degrade-quality-but-do-not-stop-evaluation]] defines what happens when the fast thread encounters errors, and [[load-shedding-preserves-the-critical-set-exactly-while-degrading-non-critical-equations]] handles deadline overruns, the thread pool prevents the category of violation that neither error handling nor load shedding can address: blocking the callback indefinitely on an external engine's internal mutex.
...
- [[onnx-runtime-is-fastest-for-stateless-neural-models-while-libtorch-is-fastest-for-stateful-models]] — ONNX Runtime's speed advantage for stateless models compounds with its exclusive access to ThreadPool parallelism; the engine that is fastest per-inference is also the only engine whose models can run concurrently
