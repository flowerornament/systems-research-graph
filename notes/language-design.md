...
### Tidal: Live Coding and Pattern Design (McLean 2014)
- [[live-coding-performance-requires-three-distinct-feedback-loops-that-together-define-being-in-time]] -- McLean's taxonomy: manipulation (code<>programmer), performance (sound<>programmer), social (audience<>programmer); all three must be active for "being in time"; each imposes distinct latency requirements on the composition layer
- [[functional-paradigm-dominates-live-coding-practice-because-it-provides-terseness-and-composability-at-performance-speed]] -- empirical observation: most live coding environments are functional/Lisp-based, not dataflow patcher; terseness and composability under performance pressure explain the asymmetry
- [[representing-musical-pattern-as-a-function-from-time-to-events-unifies-discrete-and-continuous-patterns]] -- Tidal's `Pattern a = Arc -> [Event a]`; querying with arc returns discrete events or samples midpoint for continuous; single type handles both, all combinators work on both
- [[rational-time-representation-eliminates-rounding-errors-in-rhythmic-subdivision]] -- `type Time = Rational`; 1/3 + 1/3 + 1/3 == 1 exactly; floating-point time corrupts triplets and polyrhythms in long-running systems
- [[cyclic-time-with-rational-subdivision-provides-the-semantic-anchor-for-pattern-operations-in-electronic-music]] -- whole numbers are successive downbeats (sam), rational points are subdivisions; reversal, phase-shifting, every-N-cycles transformations all derive semantic meaning from the cycle as unit
- [[applicative-functor-lifts-binary-value-functions-to-pattern-combinators-by-matching-co-occurring-events]] -- `Pattern` as Applicative; `blend 0.5 <$> p1 <*> p2` matches left-pattern events with right-pattern events whose arcs contain the left onset; any binary value function becomes a pattern combinator for free
- [[low-cognitive-load-and-high-viscosity-in-a-composition-language-enable-creative-flow-in-live-performance]] -- Green's cognitive dimensions applied to music: Tidal explicitly targets minimal mental overhead and maximum expressiveness-per-keystroke to support flow states during performance
- [[a-haskell-embedded-dsl-can-be-learned-and-used-creatively-without-any-functional-programming-background]] -- survey of 15 Tidal users: 12/15 no FP experience; 14/15 could make music; 11/15 found it not difficult; DSL abstraction, not host language complexity, determines accessibility
- [[iterative-redesign-of-core-representations-through-live-performance-is-the-correct-evaluation-methodology-for-music-programming-languages]] -- Tidal went through 5+ major representation rewrites, each evaluated through live performance use; performance pressure is the correct evaluation context for music language design
...
- [[mclean-2014-making-programming-languages-to-dance-to]] -- Alex McLean, FARM 2014: Tidal 0.4 design; live coding feedback loop taxonomy, pattern-as-function representation, rational time, Applicative combinators, user study showing EDSL accessibility without FP background
