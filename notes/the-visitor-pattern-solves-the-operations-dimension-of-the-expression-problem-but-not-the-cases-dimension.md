...
The algebraic effects lens ([[algebraic-effects-implement-domain-specific-dsls-by-separating-the-operation-interface-from-evaluation-strategy]]) reframes the same problem: effect operations are new "cases" added to the computation algebra; effect handlers are new "operations" (interpretations) over those cases. The freedom to add both without recompiling follows from the free-model structure -- the same categorical property that Wadler's virtual-type solution achieves through type system mechanisms. Both approaches reach the same goal via different levels of abstraction.
