...
Steele's [[the-minimum-extensibility-mechanism-for-user-driven-language-growth-is-generic-types-and-operator-overloading]] proposes a statically-typed analogue of this same idea for Java: generic types + operator overloading together allow user-defined numeric types to participate in the same dispatch as built-in types, without sacrificing static safety. Where Sussman's generic dispatch is runtime and open-world, Steele's proposal is compile-time and type-system-constrained -- but both target the same goal: user-defined types that are indistinguishable from primitives in use.
