...
**I_max must be measured, not derived.** Because [[inference-engines-violate-real-time-principles-on-every-inference-not-just-initial-ones]], the worst-case inference time cannot be computed analytically — non-deterministic allocations and syscalls make runtime distributions unpredictable from source inspection. It requires empirical benchmarking. Anira's built-in benchmarking infrastructure addresses this. The measurement should run post-warmup (see [[warm-up-inferences-before-the-audio-callback-stabilize-inference-engine-runtimes]]) to capture stabilized runtimes rather than cold-start outliers. Engine choice is itself a lever on I_max: since [[onnx-runtime-is-fastest-for-stateless-neural-models-while-libtorch-is-fastest-for-stateful-models]], selecting ONNX Runtime for stateless models reduces I_max and thereby reduces L_total across all buffer sizes.

The H_host parameter in these equations is exactly the murail block size N described in [[block-size-is-the-primary-latency-throughput-tradeoff-in-murail-execution]]: the same parameter that controls latency-throughput tradeoff in the host audio engine also appears as the quantization unit in the neural inference latency formula. Increasing H_host improves per-sample inference throughput (fewer callback invocations) but increases L_total because the ceil() rounding grows coarser. This means the host engine's block size choice and the neural inference latency budget are not independent -- they are coupled through the same variable.
...
The formula also provides quantitative grounding for why [[autoregressive-synthesis-prevents-real-time-audio-generation-at-usable-sample-rates]]: an autoregressive model generating at 57Hz on CPU has I_max ≈ 840 samples at 48kHz, which yields L_total = ceil(840 / H_host) * H_host — multiple host buffer periods of added latency that no practical audio application can absorb. Architectural alternatives work precisely because they reduce I_max to a range where the formula produces tolerable L_total values: [[multiband-decomposition-reduces-temporal-dimensionality-enabling-real-time-neural-audio-at-48khz]] reduces I_max by shrinking the temporal dimension the model operates on, while parallel decoders reduce I_max by removing per-sample sequential dependencies.
