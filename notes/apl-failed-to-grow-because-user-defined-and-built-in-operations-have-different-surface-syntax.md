...
The live coding domain provides a contemporary datapoint: [[functional-paradigm-dominates-live-coding-practice-because-it-provides-terseness-and-composability-at-performance-speed]] observes that functional/Lisp languages dominate live performance precisely because they satisfy the seam-free criterion -- user-defined operations (pattern transformers, combinators) are syntactically indistinguishable from built-ins. Tidal's pattern library and ChucK's built-in UGens both allow user extensions to compose freely alongside system-provided primitives. APL's equivalent -- a live-coded notation language where user patterns looked visually different from system patterns -- would have been unusable for the rapid iteration live performance requires.
