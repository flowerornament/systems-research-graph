...
**Why deep handlers are the default**: deep handlers correspond to the original categorical interpretation of effect handlers as a fold over the free monad algebra (Plotkin and Pretnar 2009). They give cleaner semantics for recursive effect processing -- an iterator that yields values is naturally modeled as a deep handler (each yield goes to the same foreach logic, not escaping). Shallow handlers are useful when the handler needs to inspect what effect is issued and then delegate to a different handler.
