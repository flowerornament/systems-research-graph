...
### Tidal Cycles: Time Model and Pattern Representation (McLean 2014)
- [[rational-time-representation-eliminates-rounding-errors-in-rhythmic-subdivision]] -- `type Time = Rational`; arithmetic over triplets and polyrhythms is exact; floating-point accumulation errors corrupt long-running patterns; the scheduler should use rational time (or fixed-denominator integer equivalents) not float
- [[cyclic-time-with-rational-subdivision-provides-the-semantic-anchor-for-pattern-operations-in-electronic-music]] -- whole numbers as downbeats, rationals as subdivisions; reversal, phase-shift, and conditional transformations all derive semantic meaning from cycles; aligns with Indian classical sam and EDM downbeat traditions
- [[representing-musical-pattern-as-a-function-from-time-to-events-unifies-discrete-and-continuous-patterns]] -- single Pattern type handles both event sequences and continuous signals by querying arcs; all combinators compose freely without type-case branching; candidate design for murail's composition language time-varying value type
