...
Wadler's 1998 [[the-expression-problem-names-the-tension-between-adding-new-cases-and-new-operations-without-recompiling]] offers a complementary counterpoint: with sufficiently expressive type systems (virtual type indexing + F-bounded polymorphism in GJ), both extensibility dimensions can be achieved *with* static type safety -- suggesting the tradeoff Sussman identifies is not universal but type-system-dependent. [[the-expression-problem-requires-static-type-safety-and-independent-compilation-simultaneously]] sets the constraint precisely: the question is whether the type system is expressive enough to avoid the tradeoff entirely.
